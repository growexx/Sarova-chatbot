...
from code_modules.llm_response_extractor import LLMResponseExtractor , smart_reorder
...
_ALL_TABLES = ["gl_audit_data", "user_audit_data", "ap_audit_data"]
...
class ChatService:
    ...
    def _run_text_2_sql(self, user_message: str, last_sql_query: str) -> dict:
        ...
        extractor = LLMResponseExtractor()
        extractor.set_data(raw)
        sql_query, error_status = extractor.get_many(
            ["sql_query", "error_flag"],
            {"sql_query": "", "error_flag": 0}
        )
        return {"sql_query": sql_query, "error_status": error_status}

    @staticmethod
    def _prepare_local_file_and_par_url(file_prefix:str = "result_data", bucket_folder_name: str = "db_data_excel_wise/")  -> Tuple[str, str]:
        ...
        par_url=f"https://objectstorage.eu-frankfurt-1.oraclecloud.com/p/wtui-hBFp4lw-bfvg4jD3LBcDaazKo1Dj7D_WsxCkXu0AA1wZhBq4EN9whdVC2Zw/n/frqtscc9uxjn/b/americana-poc-bucket/o/{object_name}"
    ...
    async def handle_inquiry(self, user_id: str, chat_id: str, user_message: str, app_state):
        ...
        try:
            ...
            print(f"[SQL Gen] Generated SQL: {sql_query}")
            ...
            if len(chat_history) > 5:
                ...
                print(f"chat history exceed 5 messages , Aftre removal {chat_history}")
            ...
            if num_of_records <= 100:
                final_response = self.prepare_data_response(
                    selected_df, sql_query, message , None
                )
            else:
                ...
                final_response = self.prepare_data_response(
                    None, sql_query, message , actual_par
                )
    ...
    @staticmethod
    def prepare_data_response(selected_df: pd.DataFrame, sql_query: str, message: str,par: Optional[str] = None):
        """
